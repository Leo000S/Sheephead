...
# --- Hilfsfunktionen für Rechteprüfungen ---
def ist_admin_oder_boss(group_data, user_id):
    """Gibt True zurück, wenn der User Boss oder in der Admin-Liste ist."""
...
def group_menu(user):
    ...
    with t2:
        # =====================================================================
        # TAB 2: Gruppenverwaltung
        # =====================================================================
        # Alle Gruppen laden, bei denen der User Admin oder Boss ist
        ...
        if not meine_verwalteten_gruppen:
            st.info("Du bist aktuell weder Admin noch Gründer einer Gruppe.")
        else:
            ...
            # --- 10-TAGE AUTOMATISMUS & BEITRITTSANFRAGEN ---
            st.write("### 📥 Offene Beitrittsanfragen")
            ...
            if not anfragen:
                ...
                for req in anfragen:
                    ...
                    # Berechnen, wie viele Tage vergangen sind
                    ...
                    antragsteller_name = all_p.get(antragsteller_id, "Unbekannter Spieler")

                    # Logik: Keine Ablehnung nach 10 Tagen = Automatisches Ja
                    if tage_alt >= 10:
            ...
            # 1. Turniere aus DB laden & parsen
            ...
            with col2:
                ...
                # 3. Live-Status & Buttons (Aus der Einrückung geholt, damit immer sichtbar!)
                start_tag, end_tag = "", ""
                ...
                with btn_col2:
                    if bearbeitungs_modus:
                        ...
                        # Platzhalter, damit das Layout sauber bleibt, wenn kein Lösch-Button da ist
                        st.write("")

            # --- GEFAHRENBEREICH (Nur Gründer/Boss) ---
            if ist_mein_boss:
                st.divider()
                with st.expander("⚠️ Gruppe unwiderruflich auflösen"):
                    st.warning(
    ...
    with t3:
        ...
        if not meine_einladungen:
            st.write("_Du hast aktuell keine offenen Einladungen._")
        else:
